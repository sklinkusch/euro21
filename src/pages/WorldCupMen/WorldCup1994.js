...
import matches, { matchaf, matchvf, matchhf, match3p, matchf } from "../../data/WorldCupMen/matches_wc1994"
...
import { getAssociation20, getKoTeams, getKoTeam, FlagSet, getChampion, getLoser } from '../../components/helpers';
import { Finale, Platz3 } from '../../components/Finale';
import { localeName as locales } from '../../components/countries';
/* eslint-disable react-hooks/exhaustive-deps */

function WorldCup1994() {
  ...
  const [teams3p, setTeams3p] = useDebugState("3pTeams",[])
  const [fTeams, setFTeams] = useDebugState("fTeams",[])
  const [champion, setChampion] = useDebugState("champion",null)
  useEffect(() => {
    document.title = `${locales("WorldCup")} 1994`
  }, [])
  useEffect(() => {
    const aTeamAF1 = first[2]
    const bTeamAF1 = winningTeams[association.indexOf(3)]
    const teamsAF1 = [aTeamAF1, bTeamAF1]
    const aTeamAF2 = second[0]
    const bTeamAF2 = second[2]
    const teamsAF2 = [aTeamAF2, bTeamAF2]
    const aTeamAF3 = second[5]
    const bTeamAF3 = second[1]
    const teamsAF3 = [aTeamAF3, bTeamAF3]
    const aTeamAF4 = first[0]
    const bTeamAF4 = winningTeams[association.indexOf(7)]
    const teamsAF4 = [aTeamAF4, bTeamAF4]
    const aTeamAF5 = first[5]
    const bTeamAF5 = second[4]
    const teamsAF5 = [aTeamAF5, bTeamAF5]
    const aTeamAF6 = first[1]
    const bTeamAF6 = winningTeams[association.indexOf(5)]
    const teamsAF6 = [aTeamAF6, bTeamAF6]
    const aTeamAF7 = first[3]
    const bTeamAF7 = winningTeams[association.indexOf(2)]
    const teamsAF7 = [aTeamAF7, bTeamAF7]
    const aTeamAF8 = first[4]
    const bTeamAF8 = second[3]
    const teamsAF8 = [aTeamAF8, bTeamAF8]
    const teamsAF = [teamsAF1, teamsAF2, teamsAF3, teamsAF4, teamsAF5, teamsAF6, teamsAF7, teamsAF8]
    setAfTeams(teamsAF)
  })
  useEffect(() => {
    const aTeamVF1 = getKoTeams(matchaf, 6, first[3], winningTeams[association.indexOf(2)])
    const bTeamVF1 = getKoTeams(matchaf, 1, second[0], second[2])
    const teamsVF1 = [aTeamVF1, bTeamVF1]
    const aTeamVF2 = getKoTeams(matchaf, 5, first[1], winningTeams[association.indexOf(5)])
    const bTeamVF2 = getKoTeams(matchaf, 4, first[5], second[4])
    const teamsVF2 = [aTeamVF2, bTeamVF2]
    const aTeamVF3 = getKoTeams(matchaf, 7, first[4], second[3])
    const bTeamVF3 = getKoTeams(matchaf, 0, first[2], winningTeams[association.indexOf(3)])
    const teamsVF3 = [aTeamVF3, bTeamVF3]
    const aTeamVF4 = getKoTeams(matchaf, 3, first[0], winningTeams[association.indexOf(7)])
    const bTeamVF4 = getKoTeams(matchaf, 2, second[5], second[1])
  ...
  useEffect(() => {
    ...
    const bTeamHF2 = getKoTeam(matchvf, 1, vfTeams[1], 0, vfTeams[1], 1)
  ...
  useEffect(() => {
    ...
    const aTeamF = getKoTeam(matchhf, 1, hfTeams[1], 0, hfTeams[1], 1)
    const bTeamF = getKoTeam(matchhf, 0, hfTeams[0], 0, hfTeams[0], 1)
    const teams3P = [aTeam3p, bTeam3p]
